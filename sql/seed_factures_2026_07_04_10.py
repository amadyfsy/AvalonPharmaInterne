#!/usr/bin/env python3
"""Ajoute / resynchronise les factures 2026/07/04 à 2026/07/10 (sauf 07/06 déjà couverte).

Usage (PythonAnywhere — activer le venv d'abord) :
  source ~/.virtualenvs/avalon-interne/bin/activate
  python sql/seed_factures_2026_07_04_10.py
"""
...
FACTURES = [
    ...
    },
    {
        ...
        "client_aliases": ("MEDICAL DISTRIBUTION",),
    ...
    },
    {
        ...
        "client_type": "hopital",
        "client_aliases": ("CHR Saint Louis", "CHR de Saint-Louis"),
        "lignes": [("Miroir de Clar", 2, 145000)],
        "total_attendu": 290000,
    },
]

PRODUIT_ALIASES = {
    "hystero": ("Hystéro", "Hystero"),
    "valves d heimlich double": ("Valves d'Heimlich double",),
    "surgicel": ("Surgicel",),
}
...
def find_client(Client, name: str, aliases: tuple[str, ...] = ()):
    ...
    if "rajunt" in name_n:
        for c in Client.query.all():
            if "rajunt" in _norm(c.raison_sociale or ""):
                return c
    if "medical distribution" in name_n:
        for c in Client.query.all():
            rs = _norm(c.raison_sociale or "")
            if "medical" in rs and "distribution" in rs:
                return c
    if "matar lo" in name_n:
        for c in Client.query.all():
            if "matar" in _norm(c.raison_sociale or ""):
                return c
    if "saint" in name_n and "louis" in name_n:
        for c in Client.query.all():
            rs = _norm(c.raison_sociale or "")
            if "saint" in rs and "louis" in rs:
                return c
    return None
